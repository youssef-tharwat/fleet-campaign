...
Read both self-contained role briefs before dispatch:

- [Best practices reviewer](references/best-practices-reviewer.md): design,
  duplicated knowledge, ownership boundaries, clarity and project idioms.
- [Senior code reviewer](references/senior-code-reviewer.md): correctness,
  regressions, contracts, concurrency, persistence and production risks.

These briefs are portable prompts, not requirements for registered agent types.
When Fleet Campaign has authorized reviewer capacity, use Herdr to dispatch two
separate read-only review agents. Pass each its own brief, the exact
root/revision/diff, relevant instructions, evidence gaps, and a read-only
assignment. Do not share the other lens's conclusions before both return. Inherit
the user's model choices; no particular model, vendor, or graph service is
required.
